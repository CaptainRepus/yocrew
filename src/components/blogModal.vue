<template>
  <ion-modal :is-open="isOpen" @will-dismiss="closeModal" class="custom-modal">
    ...
    <ion-content class="ion-padding">
      <h1 class="text-2xl font-bold mb-1 text-center">{{ blog.name }}</h1>
      <h3 class="text-lg mb-5 text-center">Autor: {{ blog.autor }}</h3>
      ...
      <div class="w-full h-1/3 md:h-2/3 rounded-xl border border-green-500 overflow-hidden mb-5">
        <img :src="blog.cesta_obrazku" class="w-full h-full"></img>
      </div>
  ...
  methods: {
    ...
    partTheBlog() {
      if (this.blog && this.blog.description) {
        const parts = this.blog.description.split('.').map(part => part.trim()).filter(part => part);
        if (parts.length > 1) {
